...
def render_category_page(slug: str, label: str, events: list) -> str:
...
@app.command()
def main(
    min_events: int = typer.Option(3, "--min-events", help="No genera página para categorías con menos eventos próximos que esto (evita contenido delgado)"),
    ...
    hub_entries = []
    sitemap_urls = [f"{BASE_URL}/", f"{BASE_URL}/categoria/"]
    written = 0
    for slug, label in CATEGORY_META.items():
        evs = by_category.get(slug, [])
        n = len(evs)
        if n < min_events:
            print(f"⏭️  {slug:<14} {n:>3} eventos — por debajo del mínimo ({min_events}), sin página")
            continue
        print(f"✅ {slug:<14} {n:>3} eventos — {'(dry-run, no escribe)' if dry_run else 'generando página'}")
        hub_entries.append((slug, label, n))
        sitemap_urls.append(f"{BASE_URL}/categoria/{slug}/")
        if not dry_run:
            out_dir = SITE_DIR / "categoria" / slug
            out_dir.mkdir(parents=True, exist_ok=True)
            (out_dir / "index.html").write_text(render_category_page(slug, label, evs), encoding="utf-8")
            written += 1

    if dry_run:
        print(f"\n[dry-run] Generaría {len(hub_entries)} páginas de categoría + hub + sitemap. Nada escrito.")
        return
