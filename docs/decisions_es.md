...
2. ✅ **Hecho, con una vuelta (2026-08-15):** el primer intento (`<> 'city_hint_only'`) resultó insuficiente — Diego re-corrió el export y persistían 92 eventos repartidos en 4 coordenadas "promiscuas" (una sola coordenada compartida por 20-33 `locationName` completamente distintos, incluyendo `@dichaparis`, "Teatro Colsubsidio Roberto Arias Pérez" en Medellín, o directamente handles de Instagram). Diagnóstico con la query de verificación (`MATCH (l:Location) WHERE l.lat=... AND l.lon=... RETURN l.name, l.geocodeConfidence, l.displayName`): en los 40 casos revisados, `geocodeConfidence` era **`null`**, no `"city_hint_only"` — y `l.displayName` era idéntico en todos: `"París, Isla de Francia, Francia metropolitana, Francia"`, confirmando que geocodificaron literalmente el `--city-hint` por defecto. Estos `Location` nunca pasaron por el sistema de tiers (geocodificados antes de que existiera ese campo, o por otra ruta del pipeline que no lo escribe). **Fix corregido:** allowlist en vez de denylist — `AND l.geocodeConfidence IN ['exact', 'city_combined']` en `EVENTS_QUERY` (`5_export_dashboard_data.py`), que excluye tanto `city_hint_only` como (sobre todo) `null`. Pendiente: Diego re-corre el export una vez más y confirma que esos 92 eventos salen del JSON — esperable una baja notable en el conteo total de eventos (170 → menos), que es el trade-off correcto (precisión sobre cobertura, ver \S de límites del método en la tesis).
