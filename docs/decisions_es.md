...
4. 🆕 **Bug encontrado (2026-08-15, no arreglado todavía):** los contadores de las pills de categoría/zona/"Todo" en `renderFilterBar()` (`site/app.js`) se calculan sobre `DATA.events` completo (todos los eventos, pasados incluidos), no sobre lo que realmente pasa el filtro de fecha activo (`STATE.when`). Por eso una categoría puede mostrar "Cine: 6" aunque los 6 eventos de cine ya hayan pasado y no aparezcan en ningún lado de la vista por defecto ("Próximos"). **Diseño propuesto por Diego:** los contadores y el menú de categorías deberían reflejar solo eventos por venir; los eventos pasados se agrupan en un bucket separado ("Pasados"), sin clasificar por tema — simplemente una lista aparte, no una categoría más del menú dinámico. Pendiente de implementar: (a) que `renderFilterBar()` calcule `zoneCounts`/`themeCounts` sobre eventos futuros/de hoy, no sobre `DATA.events` crudo; (b) agregar una opción "Pasados" al filtro de fecha que, al activarse, ignore el filtro de tema y muestre todo lo pasado sin clasificar.
