...
2. ✅ **Hecho (2026-08-15):** excluir `geocodeConfidence="city_hint_only"` del export — agregado `AND coalesce(l.geocodeConfidence, '') <> 'city_hint_only'` a `EVENTS_QUERY` en `5_export_dashboard_data.py`. Pendiente: Diego re-corre el export y confirma que Manizales/Medellín/Pereira desaparecen del JSON (o, si tienen geocodificación `exact`/`city_combined` real, aparecen con coordenadas correctas en vez de las de París).
