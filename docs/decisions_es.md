...
**DD-074 — Cinco correcciones sobre DD-073, desde una captura real de Diego (2026-08-29).** Diego probó el menú en cascada recién desplegado y reportó, con una captura del sitio en francés: (1) los pills de geo ("Francia"/"Fuera de Francia") se veían en español pese a tener el sitio en FR; (2) pidió que "Dónde" deje de ocupar espacio en el filterbar y se vuelva un menú desplegable junto a ES/FR; (3) confusión sobre por qué "Francia" mostraba menos eventos que un pill que él leía como "Île-de-France"; (4) la fila "Gratis/Cerca de mí/Ordenar" dejaba mucho espacio vacío; (5) el hero de portada siempre mostraba el mismo evento. En un mensaje de seguimiento agregó un sexto punto: las categorías también deberían depender del país/ciudad activo.

- **(1) Bug de i18n, real, mío:** al armar `GEO_FILTERS` en DD-073 le puse un campo `label` fijo en español a cada entrada, sin pasar por el sistema de traducción — a diferencia del resto del sitio (categorías, meses, geoZone real), que sí pasa todo por `I18N[CURRENT_LANG]`. Fix: se retiró `label` de `GEO_FILTERS` (solo queda `parent`/`match`) y el texto se resuelve siempre vía `geoLabel(key)`, que ya existía para las etiquetas de `geoZone` real — se le agregaron entradas nuevas a `geoLabels` (ES y FR) para las claves agregadas del menú (`francia`, `fuera-de-francia`, `ile-de-france`, `fuera-de-ile-de-france`, `paris`), que conviven sin pisarse con las claves de geoZone real porque son strings distintos.
- **(3) La "confusión" era el mismo bug, con un agravante:** el string `geoAll` (el pill de "Todos") databa de antes del rediseño en cascada y decía literalmente "Toda Île-de-France y más" / "Toute l'Île-de-France et plus" — un texto que dejó de tener sentido en cuanto el menú pasó a tener niveles reales de país/zona/ciudad. Diego lo leía como si fuera el conteo de Île-de-France. Corregido a "Todos"/"Tous". La exclusividad de "Francia" que Diego pedía YA estaba bien implementada (el predicado de `GEO_FILTERS.francia` nunca matchea `geoZone === "Fuera de Francia"`, confirmado con tests) — no hizo falta ningún cambio de lógica ahí, solo de etiqueta.
- **(2) "Dónde" pasa de 3 filas de pills a un menú desplegable en el header**, junto al toggle ES/FR (confirmado con Diego: lista plana con indentación visual para la jerarquía, no un selector de 2 pasos). Nuevo `#geo-dropdown` en `site/index.html` (botón + panel), `renderGeoDropdown()` en `app.js` arma las 6 opciones (`GEO_MENU_ITEMS`, con su nivel de indentación) cada render, `setGeo()` (hoisteada a nivel de módulo, antes vivía como closure adentro de `renderFilterBar()`) centraliza el `pushState` a la URL bonita correspondiente (DD-073) igual que antes. Cierre con click afuera o Escape, registrado una sola vez a nivel de documento. `.topnav-inner` ganó un contenedor `.topnav-right` nuevo (antes solo tenía 2 hijos con `justify-content: space-between`; agregar un tercero ahí directo los habría repartido con espacios iguales en vez de agrupar dropdown+idioma a la derecha).
- **(4) Gratis/Cerca de mí/Ordenar ya no tienen fila propia** — se movieron a la misma fila que "Cuándo". La causa real del espacio vacío no era solo la fila de más: `.sort-control` tenía `margin-left: auto`, empujándolo al extremo derecho de su fila — se retiró ese margen, ahora queda inline después de los toggles.
- **(5) Rotación del hero:** antes `hero = sorted[0]` siempre — si un evento dominaba el ranking por margen amplio, quedaba de destacado sesión tras sesión. Ahora se elige al azar entre los primeros 8 candidatos (`heroPickIndex`, variable de módulo) — sorteado una única vez por visita (primer `render()` de la carga de página) y reutilizado en los renders siguientes, para que no salte cada vez que el visitante toca un filtro; con `Math.min(heroPickIndex, sorted.length - 1)` como red de seguridad si un filtro después reduce el set por debajo del índice sorteado. `rest` (para "Destacados"/"Gratis" debajo) excluye el índice elegido, no solo el primero.
- **(6) Categorías scopeadas por geo:** `themeCounts`/el pill "Todo" de categoría se calculaban sobre `upcomingEvents` completo, ignorando `STATE.geo` — con Francia o Paris seleccionado, seguían mostrando categorías/conteos del catálogo entero. Fix de una línea: nuevo `geoScopedUpcoming` (filtra por `geoMatches` cuando `STATE.geo !== "all"`) reemplaza a `upcomingEvents` en esa sección. `applyFilters()` (la función que realmente arma la lista de eventos mostrados) ya combinaba geo y tema correctamente desde DD-073 — el gap era solo visual, en los conteos de los pills.
- Verificado: `node --check`, balance de llaves en `style.css` (125/125), grep confirmando cero referencias huérfanas a los ids/clases retirados (`geo-pills`, `geo-subpills`, `geo-subsubpills`, `filter-row-tools`), un test de Node de la lógica de selección del hero (pool grande, índice fuera de rango tras un filtro, sorteo repetido 200 veces siempre en rango, caso de un solo evento), corrida real de `6_generate_seo_pages.py` confirmando que las 4 copias `/fr/...` siguen siendo idénticas a `site/index.html` tras el cambio de header, `HTMLParser` sobre `index.html` y una copia `/fr/...`. **No visto en un navegador real** — en particular el dropdown (apertura/cierre, click afuera, Escape) y la rotación del hero (que por diseño no se nota en una sola carga de página) no se pueden verificar sin un navegador de verdad; falta que Diego lo confirme después de desplegar.

---

**DD-075 — Filterbar de una sola línea: Cuándo y Categoría se vuelven dropdowns, colores de acento por sección, pills con bordes finos (2026-08-29).** Diego probó el sitio desde el celular y reportó que el menú seguía ocupando ~70% de la pantalla pese al trabajo de DD-074 (que solo movió "Dónde" a dropdown) — dio como referencia Meetup, donde "el menú no es estático, solo aparece en la parte de arriba". Pidió generalizar el patrón: Cuándo y Categoría también pasan a dropdown, todo (buscador + 3 dropdowns) cabe en una sola línea del filterbar, y como la nube de pills de colores de Categoría desaparece de la vista permanente, usar color para "delimitar espacios" en su lugar. Agregó dos pedidos de diseño: pills con bordes finos y esquinas cuadradas (no curvas), y la misma paleta de colores pero con tonos un poco más vivos.

- **Generalización del patrón de dropdown (DD-074 lo había hecho solo para geo):** `DROPDOWN_IDS = ["when", "theme", "geo"]` y funciones compartidas `closeAllDropdowns(exceptId)`/`toggleDropdown(id)`/`initDropdownButtons()` en `site/app.js`, reemplazando las versiones geo-específicas (`closeGeoDropdown`/`toggleGeoDropdown`/`initGeoDropdownButton`) de DD-074. El listener de click-afuera y Escape ahora revisa los tres wrappers en vez de uno solo. `renderWhenDropdown()` es nuevo (mismas opciones/conteos que ya existían como fila de pills — hoy, este finde, esta semana, este mes, próximos, pasados — ahora como lista de selección única). `renderThemeDropdown()` es nuevo también, recibe `geoScopedUpcoming` (mismo scoping por geo de DD-074, punto 6) y arma la lista de categorías con >= `MIN_THEME_COUNT` eventos; cada ítem conserva un swatch de color (`CATEGORY_META[theme].color`) para no perder la señal visual que antes daba el borde/fondo de la pill, ya que el dropdown solo muestra la nube de colores cuando está abierto, no todo el tiempo.
- **Una sola fila en el filterbar:** `site/index.html` reescribe `.filterbar` a un `<input>` de búsqueda + 3 `.menu-dropdown` (Cuándo/Categoría/Dónde, cada uno con clase `accent-when`/`accent-theme`/`accent-geo`) + 2 botones de solo ícono (`#free-toggle`/`#geo-locate-toggle`, con `title`/`aria-label` que antes eran texto visible) + un `<select>` compacto (`#sort-select`). Se retiraron las filas viejas (`#when-pills`, `#theme-pills`, `.theme-row`, la etiqueta "Categoría", el wrapper `.sort-control`) y el header volvió a sus 2 hijos originales (brand + ES/FR) porque el dropdown de geo se mudó de vuelta al filterbar, junto a sus hermanos nuevos, en vez de vivir solo, aparte, en el header. `.filterbar` pasa de `flex-direction: column` (filas apiladas) a una fila (`display:flex`) con `overflow-x: auto` como red de seguridad — en pantallas muy angostas la fila scrollea horizontal en vez de romper a una segunda línea, así el alto se mantiene constante siempre, a diferencia de la vieja regla de DD-055 (`max-height:70vh` + scroll vertical en `.filterbar-wrap`) que ahora quedó obsoleta y se retiró.
- **Colores de acento por sección (`--accent-when`/`--accent-theme`/`--accent-geo` en `:root`):** ámbar para tiempo, violeta para categoría, verde azulado para geografía — asociación elegida para no pisarse entre sí ni con los colores de `CATEGORY_META`. Cada dropdown cerrado muestra un borde e ícono teñidos con su acento; el panel abierto lleva un borde superior del mismo color — el mecanismo de "delimitar espacios" que pidió Diego, reemplazando la señal que antes daba la nube de pills de colores siempre visible de Categoría.
- **Paleta de categorías más viva:** recalculada programáticamente (Python, `colorsys`, hex→RGB→HLS), saturación subida con una fórmula con techo (`min(0.72, s*1.30+0.06)`), luminosidad preservada a propósito para no romper el contraste de texto que ya dependía de los valores originales en otros usos (badges, chips). De paso se corrigió una inconsistencia de datos preexistente: "escenico" (Teatro y danza, categoría activa con volumen real) compartía el mismo gris apagado que "politico" (0 eventos) porque ninguna de las dos tenía identidad de color propia — se le dio a "escenico" un tono propio (rosa/magenta) en vez de simplemente vivificar el mismo gris para las dos.
- **Pills con bordes finos, esquinas cuadradas:** en vez de tocar solo `border-radius` de la vieja clase `.pill` (que de todas formas queda retirada del todo, ver abajo), el `border-radius` chico (3px) se aplicó de entrada a todos los elementos nuevos del filterbar (`.menu-dropdown-btn`, `.menu-item`, `.icon-toggle`, `.sort-select-compact`, `.search-input`) — mismo pedido de Diego ("bordes finos, no curvos"), aplicado de forma consistente a todo el filterbar en vez de a una sola clase aislada. Los badges/chips fuera del filterbar (`.badge-free`, `.chip`, `.tag`) no se tocaron — Diego se refería puntualmente a los pills de filtro, no a esos.
- **Limpieza de código muerto:** la clase `.pill` y la función `pillEl()` quedaron sin ningún call site tras este cambio (confirmado por grep) — se retiraron del todo de `app.js`/`style.css`, junto con `.filter-row`/`.filter-label`/`.pill-row`/`.theme-row`/`.sort-control`/`.geo-dropdown`/`.geo-dropdown-btn`/`.geo-dropdown-menu`/`.geo-item`/`.topnav-right` (todas reemplazadas por las clases genéricas `.menu-dropdown*`/`.menu-item*` o ya no aplicables tras el cambio de layout del header).
- **`data-i18n-title` (atributo nuevo):** los botones de solo ícono necesitan que su `title`/`aria-label` traduzca con el toggle ES/FR, cosa que `data-i18n`/`data-i18n-placeholder` no cubrían — se agregó soporte en `applyStaticI18n()` (`site/app.js`), mismo patrón de fallback (si no hay traducción, no se toca el atributo).
- Verificado: `node --check` en `app.js`, balance de llaves en `style.css` (125/125), `HTMLParser` sobre `index.html`, grep confirmando cero referencias huérfanas a ids/clases retiradas (`when-pills`, `theme-pills`, `pillEl`, `initGeoDropdownButton`, `.pill`, `.sort-control`, `.geo-item`, `.topnav-right`), una simulación en Node con un DOM mínimo simulado ejecutando `renderFilterBar()`/`renderWhenDropdown()`/`renderThemeDropdown()`/`renderGeoDropdown()` sobre datos de prueba sin errores de runtime, corrida real de `6_generate_seo_pages.py` y `diff` byte-a-byte confirmando que las 4 copias `/fr/...` siguen siendo idénticas a `site/index.html` tras el rediseño. **No visto en un navegador real, en particular no en un celular de verdad** — el objetivo explícito de este cambio (que el menú deje de ocupar ~70% de la pantalla en mobile) no se puede confirmar visualmente desde este entorno; falta que Diego lo pruebe después de desplegar.

---

**DD-075 (adenda) — Dos bugs reales encontrados con el navegador in-app, no solo por inspección de código (2026-08-29).** Diego mandó dos capturas después del despliegue: (1) los 3 dropdowns del filterbar se veían como texto plano, sin ícono ni flecha, a diferencia del `<select>` de Ordenar; (2) el panel de detalle de un evento sin foto mostraba el círculo de color completamente vacío, sin el ícono de categoría — y ahí noté que ni el ícono del logo (`.brand-mark`) se veía. Se investigó con el navegador in-app en vez de solo revisar el código, contra el sitio ya desplegado.

- **Causa raíz real, confirmada con `fetch()` en vivo:** el `<link>` a la fuente de íconos apuntaba a `https://cdn.jsdelivr.net/npm/@tabler/icons-webfont@2/dist/tabler-icons.min.css`, que devuelve 404 ("Couldn't find the requested file /dist/tabler-icons.min.css in @tabler/icons-webfont") — la ruta real del paquete es sin `/dist/`. Esto rompía **todos** los íconos `ti-*` del sitio (logo, tarjetas, badges, panel de detalle) desde que se agregó ese link, no algo nuevo de este cambio — confirmado con `document.fonts` (la fuente "tabler-icons" nunca aparecía cargada) y verificado que la URL corregida sirve 200 con las 36 de las 37 clases `ti-*` que usa el código (`ti-ballot`, usada en la categoría "Cívico"/político, no existe en absoluto en la v2 real de Tabler — se reemplazó por `ti-flag`, que sí existe). Corregido el `<link>` en `site/index.html`. Los íconos SVG en línea del filterbar (agregados en el fix anterior de esta misma entrega) se dejaron igual — son una capa de robustez adicional que ahora es redundante con la fuente ya funcionando, pero no hace daño mantenerlos.
- **Verificado visualmente en el navegador in-app, contra el sitio en vivo:** se inyectó el `<link>` corregido sobre la página ya cargada (sin recargar) y se confirmó por captura de pantalla que el logo, el ícono de categoría dentro del círculo de color del evento destacado, el ícono "✧" del eyebrow, y los 3 íconos+flechas de los dropdowns del filterbar aparecen correctamente — algo que la sola revisión de código no podía confirmar.
- **Segundo bug, encontrado en el mismo intento de verificación visual:** al abrir uno de los dropdowns (ej. "Dónde") en el navegador real, el botón se marcaba correctamente como activo (`aria-expanded="true"`, fondo oscuro) pero el panel desplegable nunca se veía. Causa: `.menu-dropdown-menu` era `position: absolute`, y su ancestro `.filterbar` tiene `overflow-x: auto` (agregado en el rediseño de una sola línea) — un panel `position:absolute` que se extiende más abajo que su contenedor queda recortado por ese mismo `overflow`, aunque el recorte sea solo para permitir el scroll horizontal de la fila. Confirmado con `getComputedStyle`/`getBoundingClientRect` en el navegador real: el panel tenía todos los estilos "correctos" (`display:block`, `opacity:1`, `z-index:30`) pero su área visible quedaba en cero por el clipping del ancestro.
- **Fix:** `.menu-dropdown-menu` pasa a `position: fixed`, con `top`/`left` calculados a mano en JS (`positionDropdownMenu()`, llamada desde `toggleDropdown()` al abrir) contra `getBoundingClientRect()` del botón — `position:fixed` saca al panel del árbol de clipping de sus ancestros (siempre que ninguno tenga `transform`/`filter`/`contain`, verificado que no es el caso). Incluye un clamp simple: si el panel se saldría por el borde derecho del viewport, se alinea su borde derecho al del botón en vez del izquierdo. Como un panel `fixed` ya no se ancla al padre `relative` cuando la fila scrollea horizontal o la página scrollea antes de que el filterbar quede pegado (`position:sticky`), se agregó un cierre automático en scroll/resize (`document.addEventListener("scroll", ..., true)` con `capture:true`, porque el scroll de un contenedor interno no burbujea) — mismo patrón ya usado para click-afuera/Escape, en vez de recalcular la posición en cada evento.
- **Metodología, para dejar constancia:** este bug (el panel invisible) no se había detectado en las rondas de verificación anteriores de DD-075 porque esas solo cubrían `node --check`/balance de llaves/simulación de DOM en Node — ninguna renderiza layout real ni CSS de verdad. Se detectó recién al usar el navegador in-app contra el sitio desplegado, con capturas de pantalla y `getBoundingClientRect()` reales. Para cambios de layout/CSS a futuro, vale la pena priorizar antes esta verificación visual en vez de solo la sintáctica.
- Verificado: `node --check`, balance de llaves en `style.css` (126/126), fetch real contra jsdelivr confirmando 200 + las 36/37 clases, `document.fonts`/`getComputedStyle`/`getBoundingClientRect`/`elementFromPoint` en el navegador in-app contra el sitio en vivo confirmando que el panel se pinta en el pixel correcto tras el fix, capturas de pantalla confirmando íconos visibles en logo/tarjetas/hero/filterbar, corrida real de `6_generate_seo_pages.py` con diff byte-a-byte confirmando que las 4 copias `/fr/...` siguen idénticas a `site/index.html`. **Pendiente:** que Diego confirme en su celular real que el panel del dropdown se ve y se puede interactuar (la verificación en el navegador in-app usó clicks disparados por JS, no gestos táctiles reales).
